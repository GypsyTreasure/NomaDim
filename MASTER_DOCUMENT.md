...
**Dimensions (display-only in v1):** a `Measure`-style sketch readout shows live length/angle/radius of hovered or selected entities; persistent driving dimensions arrive with the v2 solver. Precision workflow in v1 = numeric input at creation + properties panel afterwards.
