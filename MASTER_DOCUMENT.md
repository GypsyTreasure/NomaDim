...
- **Offset** (W, #8/#2, ADR-0120→ADR-0134, AutoCAD-like): a parallel copy of a whole **selection**. Preselect geometry (many lines, a loop, circles/arcs), pick Offset, type the **Distance** in the parameters window and **click the side** you want (like AutoCAD's type-distance-then-pick-side); with no distance typed, the perpendicular distance from the click is used. **Connected line chains offset as a unit** — each segment is offset and adjacent offsets are intersected into clean **mitred corners**, welded into a proper parallel loop/polyline (not disjoint segments); **circles/arcs offset concentrically** (r ± distance, keeping the arc's angular extent). The copy is plain new geometry (same path as Rectangle/Mirror; construction flag inherited). Splines/points aren't offsettable in v1.
