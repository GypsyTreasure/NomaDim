...
- **Thin / single-wall (#7, ADR-0068):** both Extrude and Revolve take an optional **Wall Thickness** (mm; 0 = a solid body, the default). When set, the created feature is hollowed to that wall thickness — a single-wall shell, ideal for 3D-printed enclosures/vases — *before* the boolean, so a thin wall still Joins/Cuts/Intersects an existing body just like a solid. Implemented by reusing the Shell hollowing helper (`closedHollow`) on the built tool; walls too thick to fit collapse with a clear error.
