...
**Body cross-section on the sketch plane (ADR-0050):** while sketching, the outline where the sketch plane cuts each visible body is shown as a **display-only** violet reference (never editable, never persisted) so you can draw against existing geometry — Fusion's "Intersect" as an automatic guide. It is computed by slicing the tessellated body meshes on the main thread (no kernel round-trip), respects hidden bodies, and clears when the sketch closes.
