...
import { applyTrsf, mirrorTrsf } from './transform';
import { KernelExecError, type ExecCtx } from './types';

/**
 * Mirror executor (P1): reflects the source body across a world origin plane.
 * The reflected shape is a fresh copy (source preserved, §9); it is then either
 * stored as a new body (NewBody) or fused into the source (Join) via the shared
 * `applyBooleanResult` tail — which also heals an invalid face (ADR-0057).
 */
export function executeMirror(ctx: ExecCtx, op: MirrorOp): void {
  const { oc, bodies } = ctx;
  ...
  const trsf = mirrorTrsf(oc, op.plane);
